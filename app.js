...
// LOGICA INIZIALE: Verifica presenza stanza nell'URL
const roomFromUrl = getRoomFromUrl();
if (roomFromUrl) {
  roomInfo.innerHTML = `Stanza rilevata:<br>${roomFromUrl}`;
}
...
createBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    // Salva stanza su Firestore
    await addDoc(collection(window.chpriv.db, "rooms"), {
      roomId,
      password,
      createdBy: nickname,
    ...
    roomInfo.innerHTML = `
      ...
      Link:<br><a href="${roomLink}" target="_blank">${roomLink}</a>
    `;

    // Registra USER1 su RTDB
    const user1Ref = window.chpriv.ref(window.chpriv.rtdb, `presence/${roomId}/user1`);
    await window.chpriv.set(user1Ref, {
      nickname,
...
joinBtn.addEventListener("click", async () => {
  ...
  if (!nickname || !roomId) {
    alert("Inserisci il nickname e assicurati che l'ID stanza sia presente nell'URL");
    return;
  }

  try {
    const roomRef = window.chpriv.ref(window.chpriv.rtdb, `presence/${roomId}`);
    const snap = await window.chpriv.get(roomRef);
    const data = snap.exists() ? snap.val() : {};
    
    // Verifica numero utenti
    const userCount = Object.keys(data).length;
    if (userCount >= 2) {
      alert("ERRORE: Stanza piena o accesso non autorizzato");
      return;
    }

    // Registra USER2 su RTDB
    const user2Ref = window.chpriv.ref(window.chpriv.rtdb, `presence/${roomId}/user2`);
    await window.chpriv.set(user2Ref, {
      nickname,
      connectedAt: Date.now()
    });
